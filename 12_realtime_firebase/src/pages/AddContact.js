...
const AddContact = () => {
  ...
  const imagePicker = async (e) => {
    // TODO: upload image and set D-URL to
    //FIXME: DONE
    try {
      ...
      //refference to the fire baase
      const storageRef = await firebase.storage().ref();
      var uploadTask = storageRef
        .child("images/" + file.name)
        .put(resizedImage, metadata); // donot use this kind of file nam ein production grade
      ...
      uploadTask.on(
        ...
        // incase of error
        (error) => {
          toast("something is wrong in state chane", { type: "error" });
        },
        // one more call back
        () => {
      ...
      console.error(error);
      toast("something went wromg", { type: "error" });
    }
  };
